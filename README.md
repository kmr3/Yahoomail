...
`recipients` が送信先一覧、`sender` が差出人です。アプリ画面では、`recipients` の `label` がセレクトボックスに表示されます。
...
最初に選ばれる送信先を指定したい場合は、`app.default_recipient` に `label` と同じ名前を書きます。

```json
{
  "app": {
    "default_recipient": "夜間部"
  }
}
```
...
次のように、`夜間部` と `中間部` をキーにした書き方にも対応しています。
...
メールアドレスの項目名は `email` をおすすめします。`mail`、`address`、`メールアドレス` でも読み込めます。
...
1. 送信先を確認します。
2. 送信先を「夜間部」「中間部」から選びます。
3. 「Excelファイルを選ぶ」を押して、`.xlsx` / `.xls` / `.xlsm` のいずれかを選びます。
4. 件名と本文を確認します。
5. 「送信前に確認」を押します。
6. 内容を確認して「送信する」を押します。
